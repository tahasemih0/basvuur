...
require("./util/eventLoader")(client); //client
...
client.on("clickButton", async (button) => {
  ...
  const onaymsj = new Discord.MessageEmbed()
    .setAuthor(
      "Bot #Club",
      button.message.guild.iconURL({ dynamic: true, type: "gif", size: 1024 })
    )
    .setDescription(
      `Başvurunuz Onaylandı ve Yetkili Rolleriniz verildi, Tebrikler :)`
    )
    .setColor("GREEN");
  ...
  if (button.id === "red") {
    ...
    collector.on("end", (collected) => {
      ...
      const redmsg = new Discord.MessageEmbed()
        ...
        .setDescription(
          `<@${basvuruGonderenID}> Başvurunuz, \`${collected
            .map((m) => m.content)
            .slice(0, 1)}\` nedeniyle ${
            button.clicker.user
      ...
      button.message.edit(
        `<@${basvuruGonderenID}> adlı kişinin, Başvurusu, \`${collected
          .map((m) => m.content)
          .slice(0, 1)}\` Sebebiyle, \`${
          button.clicker.user.tag
        }\` isimli yetkili tarafından Başarıyla Reddedildi`,
        redbuton
      );
      client.channels.cache
        .get(ayarlar.onayred)
        .send(`<@${basvuruGonderenID}>,`, redmsg);
    });
  }
